...
public class UsuarioDAO {
	...
	public void update(Usuario usuario) {
		String sql = "UPDATE Cad_Usuario SET senha = ?, data_nascimento = ?, data_ciencia = ?, estado = ?, cidade = ?, salario = ?, nivelInstrucao = ?, empresa = ? WHERE nome_usuario=?";
		PreparedStatement preparador;
		try {
			preparador = con.prepareStatement(sql);
			preparador.setString(1,  usuario.getSenha());
			preparador.setString(2,  usuario.getData_nascimento());
			preparador.setString(3,  usuario.getData_ciencia());
			preparador.setString(4,  usuario.getEstado());
			preparador.setString(5,  usuario.getCidade());
			preparador.setString(6,  usuario.getSalario());
			preparador.setString(7,  usuario.getNivelInstrucao());
			preparador.setString(8,  usuario.getEmpresa());
			preparador.setString(9,  usuario.getNome_usuario());
